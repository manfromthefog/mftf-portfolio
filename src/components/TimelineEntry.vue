  ...
  <div class="flex items-start w-full mb-12 relative opacity-0 transform translate-y-10 animate-fade-in-up">
    ...
    <div class="ml-10 flex flex-col md:flex-row w-full items-center">
      ...
      <div class="w-1/2 flex flex-col items-start">
        <p class="inline-block px-2 py-1 mb-5 font-semibold text-white bg-stone-900 dark:bg-white dark:text-stone-900 rounded-md ">{{ time }}</p>
        <h2 class="text-2xl font-semibold mb-2">{{ event }}</h2>
      ...
      <div class="w-1/2 flex flex-wrap justify-center items-center md:justify-start gap-4 mt-4 md:mt-0">
        <img v-for="(image, index) in images" :key="index" :src="image" alt="Timeline Image" class="w-32 h-32 object-cover rounded-md shadow-lg polaroid" />
      </div>
    </div>
  </div>
</template>
